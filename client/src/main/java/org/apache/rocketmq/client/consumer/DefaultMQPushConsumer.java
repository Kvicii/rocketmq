...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </li>
     * </ul>
     * 根据消息进度从消息服务器拉取不同消息时重新计算消费策略
     * 如果从消息进度服务OffsetStore读取到MessageQueue中的偏移量 >= 0 使用读取到的偏移量
     * 只有在读到的偏移量 < 0时ConsumeFromWhere的策略才会生效
     */
     ...
     * 消费者最大线程数
     * 由于消费者线程池使用了无界队列 消费者消费者线程个数最多只有consumeThreadMin个
     */
     ...
     * 并发消息消费时处理队列最大跨度 如果ConsumeQueue中偏移量最大的消息与偏移量最少的消息跨度超过2000 则延迟50ms后在拉取消息
     */
     ...
     * 消息并发消费时一次消费消息条数
     * 即每次传入MessageListener#consumeMessage中的消息条数
     */
     ...
     * queue waiting.
     * 最大消费重试次数 如果消息消费次数超过maxReconsumeTimes还未成功 将该消息移入失败队列 等待被删除
     */
