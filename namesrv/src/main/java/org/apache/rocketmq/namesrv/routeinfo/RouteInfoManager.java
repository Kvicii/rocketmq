...
public class RouteInfoManager {
	...
	/**
	 * broker状态信息 namesrv每次收到心跳包时都会更新该信息
	 */
	...
	/**
	 * broker发送注册namesrv请求时 namesrv接收后 交由路由消息管理模块维护路由表的逻辑
	 *
	 * @param clusterName
	 * @param brokerAddr
	 * @param brokerName
	...
	public RegisterBrokerResult registerBroker(final String clusterName, final String brokerAddr,
		...
		try {
			try {
				// 加锁 同一时间只能由一个线程对路由消息表进行修改
				this.lock.writeLock().lockInterruptibly();
				// 维护clusterAddrTable
				// Set自动去重防止一个broker多次发送心跳接收到重复数据
				...
				if (null == brokerData) {
					registerFirst = true;
					// 首次注册 封装一个brokerData放入brokerAddrTable
				...
				// 更新brokerLiveTable 每隔30s broker发送的心跳包封装成一个BrokerLiveInfo放入Map 后一个覆盖前一个
	...
	/**
	 * namesrv其中的一个定时任务 检测broker心跳
	 */
	public void scanNotActiveBroker() {

		// 遍历brokerLiveTable 检测brokerLiveTable的lastUpdateTimestamp
		Iterator<Entry<String, BrokerLiveInfo>> it = this.brokerLiveTable.entrySet().iterator();
		while (it.hasNext()) {
			Entry<String, BrokerLiveInfo> next = it.next();
			long last = next.getValue().getLastUpdateTimestamp();
			// 120s没有收到心跳包 认为broker已经挂掉 namesrv将移除broker的路由信息并同时关闭socket连接
			if ((last + BROKER_CHANNEL_EXPIRED_TIME) < System.currentTimeMillis()) {
				// 关闭channel
				RemotingUtil.closeChannel(next.getValue().getChannel());
				it.remove();
				log.warn("The broker channel expired, {} {}ms", next.getKey(), BROKER_CHANNEL_EXPIRED_TIME);
				// 删除与该broker相关的所有路由表信息
		...
		if (brokerAddrFound != null && brokerAddrFound.length() > 0) {
			...
			try {
				try {
					 ...
					 */
					this.lock.writeLock().lockInterruptibly();
//                    1. 根据brokerAddress从brokerLiveTable filterServerTable中移除broker信息
...
//  -------------------------------------------------------------------------------------------------------------------
					...
					boolean removeBrokerName = false;
					/**
					 * 2.维护brokerAddrTable 从中取出brokerData
					 * 再从brokerData中取出并遍历brokerAddrs 剔除具体broker信息
					 */
					...
					while (itBrokerAddrTable.hasNext() && (null == brokerNameFound)) {
						...
						/**
						 * 经过上述操作后brokerAddrs如果不再含有元素 从brokerAddrTable中剔除broker信息
						 */
						if (brokerData.getBrokerAddrs().isEmpty()) {
							removeBrokerName = true;
...
//  -------------------------------------------------------------------------------------------------------------------
					/**
					 * 3.根据维护brokerAddrTable过程拿到的变量进行
					 * 实质上是维护broker和cluster之间的关系
					 */
					if (brokerNameFound != null && removeBrokerName) {
						/**
						 * 遍历clusterAddrTable 找到对应的brokerName 剔除broker
						 */
							...
							if (removed) {
								...
								if (brokerNames.isEmpty()) {
									log.info("remove the clusterName[{}] from clusterAddrTable, " +
											"because channel destroyed and no broker in this cluster", clusterName);
									it.remove();
								}
...
//  -------------------------------------------------------------------------------------------------------------------
					/**
					 * 4.根据brokerName 遍历所有TOPIC队列 如果队列中含有当前broker的队列信息则移除
					 */
					if (removeBrokerName) {
						...
						while (itTopicQueueTable.hasNext()) {
							...
							/**
							 * 操作之后的列队如果为空 从topicQueueTable中剔除改TOPIC
							 */
							if (queueDataList.isEmpty()) {
